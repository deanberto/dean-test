...
// Cursor Following Reptile Animation
class CursorFollowingReptile {
    constructor() {
        this.reptileElement = document.querySelector('.reptile-silhouette');
        ...
        this.targetX = 0;
        this.targetY = 0;
        this.ease = 0.1; // Lower values = smoother, slower following
        this.animationId = null;
        this.isReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
        
        if (this.reptileElement && this.heroSection && !this.isReducedMotion) {
            this.init();
        } else if (this.isReducedMotion) {
            console.log('Cursor following disabled due to reduced motion preference');
        }
    }
    
    init() {
        // Store original position for reference
        const rect = this.reptileElement.getBoundingClientRect();
        const heroRect = this.heroSection.getBoundingClientRect();
        
        this.originalX = rect.left + rect.width / 2 - heroRect.left;
        this.originalY = rect.top + rect.height / 2 - heroRect.top;
        this.currentX = this.originalX;
        this.currentY = this.originalY;
        
        // Add event listeners
        this.heroSection.addEventListener('mouseenter', () => this.activate());
        this.heroSection.addEventListener('mouseleave', () => this.deactivate());
        this.heroSection.addEventListener('mousemove', (e) => this.updateTarget(e));
        
        // Add click interaction for fun
        this.reptileElement.addEventListener('click', () => this.onReptileClick());
        
        // Start animation loop
        this.animate();
        
        console.log('Cursor following reptile initialized');
    }
    
    activate() {
        this.isActive = true;
        this.reptileElement.style.transition = 'none'; // Remove CSS transitions for smooth JS animation
        this.reptileElement.classList.add('following');
        this.heroSection.classList.add('cursor-active');
    ...
    deactivate() {
        this.isActive = false;
        this.reptileElement.classList.remove('following');
        this.heroSection.classList.remove('cursor-active');
        ...
        // Re-enable CSS animation after returning to position
        setTimeout(() => {
            if (!this.isActive) {
                this.reptileElement.style.transition = '';
            }
        }, 1000);
    }
    ...
    updateTarget(e) {
        ...
        // Add some constraints to keep reptile within reasonable bounds
        const padding = 150;
        const maxX = heroRect.width - padding;
        const maxY = heroRect.height - padding;
        
        this.targetX = Math.max(padding, Math.min(maxX, mouseX));
        this.targetY = Math.max(padding, Math.min(maxY, mouseY));
    ...
    onReptileClick() {
        ...
        if (this.reptileElement) {
            this.reptileElement.style.transform += ' scale(1.2)';
            setTimeout(() => {
                if (this.reptileElement && this.isActive) {
                    // Reset to current position transform
                    const translateX = this.currentX - this.originalX;
                    const translateY = this.currentY - this.originalY;
                    const deltaX = this.targetX - this.currentX;
                    const deltaY = this.targetY - this.currentY;
                    const rotation = Math.atan2(deltaY, deltaX) * (180 / Math.PI) * 0.05;
                    const distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
                    const scale = 1 + Math.min(distance * 0.0005, 0.05);
                    
                    this.reptileElement.style.transform = `
        ...
        // Show a fun message
        this.showMessage('🦎 Hello there! I\'m following your cursor!');
    }
    ...
    animate() {
        ...
        if (this.reptileElement) {
            ...
            const deltaX = this.targetX - this.currentX;
            const deltaY = this.targetY - this.currentY;
            const rotation = Math.atan2(deltaY, deltaX) * (180 / Math.PI) * 0.05; // Subtle rotation
            
            // Calculate distance for scaling effect
            const distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
            const scale = 1 + Math.min(distance * 0.0005, 0.05);
            
            // Apply the transform with the original translate(-50%, -50%) preserved
            this.reptileElement.style.transform = `
                translate(calc(-50% + ${translateX}px), calc(-50% + ${translateY}px)) 
                rotate(${rotation}deg) 
                scale(${scale})
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize cursor following reptile
    window.cursorReptile = new CursorFollowingReptile();
